...
--ch5
--列出學員姓名及各科成績
select s.id, s.name, c.name, score
...
from course
group by id
having sum(score) >= all (select sum(score) from course group by id)
...
--查出學員總分數最高 (含姓名,總分)
with sum
  as (select  s.name, sum(score) as 'sum1' 
      from student s left outer join course c
	  on s.id = c.id 
	  group by s.name )
select name , sum1
from sum
where sum1 = (select max(sum1) from sum)
  
--查出學員總分數最高 (含學號,姓名,總分)
with sum
  as (select s.id, s.name, sum(score) as 'sum1' 
      from student s left outer join course c
	  on s.id = c.id 
	  group by s.name , s.id )
select id, name , sum1
...
select s.name , score , c.name
from  student s left outer join course c
on s.id = c.id 
where score > any (select avg(score) from course )
order by s.name
--查出學員各科分數大於該科平均的有那些 (含姓名,個人分數) 
select s.name , c.name , c.score , a.avg1
from  student s join course c
on s.id = c.id 
join (select avg(score) avg1 , name from course group by name) a
on c.name = a.name
where c.score >  a.avg1
...
select * from course
pivot (sum(score) for name in ([JAVA],[T-SQL],[電概])) piv

--ch8 adv Subquery
--列出學員任一科分數及總平均分數 (含學號,個人分數,平均分數)
select id, score , (select avg(score) from course )
from course
